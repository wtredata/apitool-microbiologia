    ...
    <div class="py-3 px-2">
        <div class="col-sm-12 col-md-12">
            <div class="row note note-light note-contraste">
                <div class="col-md-10">
                    <ul id='informacion_cepa' style="list-style: none;">
                        <li><strong>Laboratorio:</strong> <span
                                class='laboratorio'>{{ $data->loteLaboratorio->first()->laboratorio->nom_laboratorio }}</span>
                        </li>
                        <li><strong>Tinsión:</strong> <span
                                class='medio'>{{ $data->tinciones->first()->nom_tincion }}</span>
                        </li>
                        <li><strong>Lote Tincion:</strong> <span class='lote'>{{ $data->lote }}</span></li>
                        <li><strong>Fecha Vencimiento Lote Tincion:</strong> <span
                                class='lote'>{{ $data->fecha_vencimiento }}</span>
                        </li>
                        @foreach ($data->LoteTincionCepas as $key => $cepas)
                            {{-- @dd($cepas->loteCepa->cepas->first()->id_cepa) --}}
                ...
                <div class="col-md-2">
                    <a href="{{ route('generar.informe.pdf', ['loteId' => $data->id, 'section' => 'Tincion']) }}"
                        class="btn btn-danger btn-block" target="_blank">Descargar pdf
                    </a>
                </div>
            </div>
        </div>
        @if ($data->LoteTincionCepas->count() < 2)
            <div class="py-3 px-2">
                <form action="{{ route('asignar.loteCepa.loteTincion') }}" method="POST">
                    @csrf
                            ...
                            <div class="col-12 mb-3">
                                <div class="row align-items-end">
                                    ...
                                    <div class="col-12 col-sm-4">
                                        ...
                                        <input type="hidden" name="loteTincionId" value="{{ $data->id }}">
        ...
        <div class="row py-3 px-2">
            @if ($data->LoteTincionCepas->count() > 0)
                @foreach ($data->comentariosLoteTincion as $ke1 => $cepas)
                    <div class="col-md">
                        <form action="{{ route('registro.cepa.tincion') }}" enctype="multipart/form-data" method="POST"
                                ...
                                <table class="table table-bordered table-sm">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                @csrf
                                                {{-- <input type="hidden" name="tincionId" value="{{ $data->id }}">
                                                <input type="hidden" name="cepaId"
                                                    value="{{$cepas->first()->lote_cepa_id}}"> --}}
                        ...
                        <table class="table table-bordered table-sm">
                            <tbody>
                                {{-- @dd($cepas) --}}
                                @foreach ($cepas->comentariosLoteCepaTincion as $cepa)
                                    {{-- @dd($cepa->tipoMedicion->nombre) --}}
                                    <tr>
                                        <form action="{{ route('update.cepa.control.tincion', ['id' => $cepa->id]) }}"
                                        ...
                                        <td>
                                            <form action="{{ route('destroy.control.cepa.tincion', ['id' => $cepa->id]) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
