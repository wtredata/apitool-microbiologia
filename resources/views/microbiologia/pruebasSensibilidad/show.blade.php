...
@section('content-section')
    <div class="py-3 px-2">
            ...
            <div class="row note note-light note-contraste">
                <div class="col-md-10">
                    <ul id='informacion_cepa' style="list-style: none;">
                        ...
                        <li><strong>Antibiótico:</strong> <span
                                class='medio'>{{ $data->pruebasSensibilidad->first()->nom_prueba }}</span>
                        </li>
                        <li><strong>Lote Antibiótico:</strong> <span class='lote'>{{ $data->lote }}</span></li>
                        <li><strong>Fecha Vencimiento Lote Prueba Sensibilidad:</strong> <span
                                class='lote'>{{ $data->fecha_vencimiento }}</span>
                        </li>
                        @foreach ($data->loteCepaPruebaSensibilidad as $key => $cepas)
                ...
                <div class="col-md-2">
                    <a href="{{ route('generar.informe.pdf', ['loteId' => $data->id, 'section' => 'Prueba']) }}"
        ...
        @if ($data->lotePruebaSensibilidadCepas->count() < 2)
            <div class="py-3 px-2">
                <form action="{{ route('asignar.loteCepa.lotePruebaSensi') }}" method="POST">
                    @csrf
                            ...
                            <div class="col-12 mb-3">
                                <div class="row align-items-end">
                                    ...
                                    <div class="col-12 col-sm-4">
                                        ...
                                        <input type="hidden" name="lotePruebaId" value="{{ $data->id }}">
        ...
        <div class="row py-3 px-2">
            @if ($data->lotePruebaSensibilidadCepas->count() > 0)
                @foreach ($data->comentariosLotePruebaSensibilidad as $ke1 => $cepas)
                    <div class="col-md">
                        <form action="{{ route('registro.cepa.prueba') }}" enctype="multipart/form-data" method="POST"
                                ...
                                <table class="table table-bordered table-sm">
                                    ...
                                    <tbody>
                                        <tr>
                                            <td>
                                                @csrf
                                                {{-- <input type="hidden" name="pruebaId" value="{{ $data->id }}">
                        ...
                        <table class="table table-bordered table-sm">
                            <tbody>
                                @foreach ($cepas->comentariosLoteCepa as $cepa)
                                    <tr>
                                        <form action="{{ route('update.cepa.control.prueba', ['id' => $cepa->id]) }}"
                                        ...
                                        <td>
                                            <form action="{{ route('destroy.control.cepa.prueba', ['id' => $cepa->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
